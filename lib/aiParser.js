...
/**
 * 使用 AI 模型解析题库文本
 * 配置：DeepSeek API - deepseek-v4-pro
...
const AI_CONFIG = {
    ...
    deepseek: {
        ...
        // DeepSeek V4 系列模型：
        //   deepseek-v4-pro   - 旗舰版，最强推理能力（推荐用于精确解析）
        //   deepseek-v4-flash - 轻量版，速度更快
...
async function parseWithAI(fullText, onProgress) {
    ...
    if (!config.apiKey || config.apiKey === 'YOUR_DEEPSEEK_API_KEY_HERE') {
        throw new Error('请配置 DeepSeek API Key（在 lib/aiParser.js 中设置）');
    }
...
function getConfigInfo() {
    const config = AI_CONFIG.deepseek;
    const hasKey = config.apiKey && config.apiKey !== 'YOUR_DEEPSEEK_API_KEY_HERE';
    return {
        provider: 'deepseek',
        model: config.model,
        configured: hasKey
...
async function askAIForExplanation(questionData) {
    const config = AI_CONFIG.deepseek;
    if (!config.apiKey || config.apiKey === 'YOUR_DEEPSEEK_API_KEY_HERE') throw new Error('DeepSeek API Key 未配置');
    ...
    content += `\n请给出详细的解析，解释知识点，并指出用户回答的对错之处（如果答错的话）。请使用 Markdown 格式组织回复，合理使用标题、列表、粗体等。`;

    const requestData = JSON.stringify({
        model: config.model,
        messages: [
            { role: 'system', content: '你是一位耐心、专业的辅导老师。请用通俗易懂的语言解析题目，回复使用 Markdown 格式。' },
